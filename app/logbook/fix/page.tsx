  ...
  return (
    <main className="mx-auto max-w-lg px-4 pb-24 pt-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link href="/logbook" className="text-sm text-air-600">로그북으로</Link>
      </div>
          ...
          {remaining.map((f) => (
            <div key={f.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <input
                ...
                placeholder="1:05"
                inputMode="numeric"
                className="w-20 rounded-lg border border-ink-line bg-white px-2 py-2 text-center font-mono outline-none focus:border-air-400"
              />
              <button
                type="button"
                onClick={() => saveRow(f)}
                disabled={hmToMin(inputs[f.id] ?? '') <= 0}
                className="rounded-lg bg-air-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-40"
              >
                저장
              </button>
            </div>
